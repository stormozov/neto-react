...
![Внешний вид компонента](./assets/docs-preview-calendar.png)
...
│   │   └── Calendar/
│   │       ├── Calendar.jsx
│   │       └── Calendar.css (опционально, можно стили в App.css)
│   ├── App.jsx
│   ├── App.css
│   └── main.jsx
├── package.json
...
* [ ] Компонент `Calendar` создан как функциональный React-компонент.
* [ ] Проп `date` передаётся и является объектом JavaScript `Date`.
* [ ] В DOM присутствует структура, соответствующая техническому заданию (см. пример HTML).
* [ ] В шапке таблицы дни недели отображены правильно, либо статично, либо с помощью `.map()` из массива.
* [ ] В таблице корректно отображаются даты текущего месяца, с правильным выравниванием по дням недели.
* [ ] Даты предыдущего месяца показаны в начале, если месяц не начинается с понедельника, и имеют класс `ui-datepicker-other-month`.
* [ ] Даты следующего месяца добавлены в конце, если месяц не заканчивается воскресеньем, с классом `ui-datepicker-other-month`.
* [ ] Текущая дата (`date`) выделена классом `ui-datepicker-today`.
* [ ] В верхнем блоке дня, месяца и года отображаются корректные русские названия с правильными падежами.
* [ ] Стили из предоставленного CSS корректно применены к компоненту.
* [ ] В коде нет синтаксических ошибок и предупреждений.
* [ ] (Опционально) Можно использовать библиотеку moment.js для работы с датами, если нужно.
